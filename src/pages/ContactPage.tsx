...
function fieldStyle(focused: boolean): CSSProperties {
  return {
    width: '100%',
    background: '#FFFDFC',
    border: `1px solid ${focused ? accent.primary : T.lineLight}`,
    ...
    <PageShell auroraTheme="general">
      <section style={{ position: 'relative', overflow: 'hidden', padding: `${T.navH + 28}px ${T.gutter} clamp(40px, 5vw, 56px)` }}>
        ...
        <div style={{ maxWidth: T.maxW, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          <FadeIn>
            <Eyebrow  accent>Contact Skylent</Eyebrow>
            ...
            {submitted ? (
              <FadeIn>
                <GlassSurface level={2} padding="clamp(32px, 5vw, 48px)" style={{ textAlign: 'center' }}>
                  <div style={{ width: 56, height: 56, borderRadius: '50%', background: `linear-gradient(135deg, ${accent.primary}, ${accent.secondary})`, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', fontSize: 22, color: C.ink, fontWeight: 700 }}>✓</div>
                  ...
                  <div style={{ background: '$rgba(11,13,15,0.04)', border: `1px solid ${T.lineLight}`, borderRadius: 10, padding: '14px 20px', display: 'inline-block', marginBottom: 20 }}>
                  ...
                  <div style={{ background: '#FFFDFC', borderRadius: 8, padding: 12, color: C.slate, fontSize: 12 }}>Demo submission — no real email was sent.</div>
                </GlassSurface>
              </FadeIn>
            ) : (
              <FadeIn>
                <GlassSurface level={2} padding="clamp(28px, 4vw, 40px)">
                  <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 700, color: C.ink, margin: '0 0 6px', letterSpacing: '-0.02em' }}>Send an enquiry</h2>
                  <p style={{ color: C.slate, fontSize: 14, margin: '0 0 24px', lineHeight: 1.6 }}>Tell us who you are and what you need. Required fields are marked.</p>
                  <form onSubmit={handleSubmit}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }} className="two-col-sm">
                      {[['Full Name', 'name', 'text', 'Your full name'], ['Email', 'email', 'email', 'your@email.com']].map(([label, field, type, ph]) => (
                        <div key={field}>
                          <label style={{ display: 'block', color: C.slate, fontSize: 11, marginBottom: 6, fontFamily: 'var(--font-mono)', letterSpacing: '0.08em' }}>{(label as string).toUpperCase()}</label>
                          <input
                            type={type as string}
                    ...
                    <div style={{ marginBottom: 14 }}>
                      <label style={{ display: 'block', color: C.slate, fontSize: 11, marginBottom: 6, fontFamily: 'var(--font-mono)', letterSpacing: '0.08em' }}>PHONE</label>
                      <input
                        type="tel"
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }} className="two-col-sm">
                      <div>
                        <label style={{ display: 'block', color: C.slate, fontSize: 11, marginBottom: 6, fontFamily: 'var(--font-mono)', letterSpacing: '0.08em' }}>I AM A</label>
                        <select
                          required
                          value={form.iam}
                      ...
                      <div>
                        <label style={{ display: 'block', color: C.slate, fontSize: 11, marginBottom: 6, fontFamily: 'var(--font-mono)', letterSpacing: '0.08em' }}>I WANT TO</label>
                        <select
                          required
                          value={form.iwant}
                    ...
                    <div style={{ marginBottom: 22 }}>
                      <label style={{ display: 'block', color: C.slate, fontSize: 11, marginBottom: 6, fontFamily: 'var(--font-mono)', letterSpacing: '0.08em' }}>MESSAGE</label>
                      <textarea
                        rows={4}
          ...
          <div>
            ...
            <FadeIn delay={100}>
              <GlassSurface level={1} padding="22px 24px">
                <div className="skylent-label" style={{ color: C.slate, marginBottom: 12 }}>Social</div>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {['LinkedIn', 'Twitter', 'YouTube', 'Instagram'].map(s => (
                    <span
                      key={s}
                      style={{ background: '$rgba(11,13,15,0.04)', border: `1px solid ${T.lineLight}`, borderRadius: 8, padding: '8px 14px', color: C.slate, fontSize: 13 }}
                    >
                      {s}
                    </span>
                  ))}
