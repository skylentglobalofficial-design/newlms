...
const glass = {
  panel: { background: C.cream, border: `1px solid ${T.lineLight}` },
  card: { background: C.cream, border: `1px solid ${T.lineLight}` },
  stat: { background: C.cream, border: `1px solid ${T.lineLight}` },
  inset: { background: C.cream, border: `1px solid ${T.lineLight}` },
}
    ...
    <div style={{ height: '80vh', display: 'flex', flexDirection: 'column' }}>
      <div style={{ background: C.ink, padding: '10px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button onClick={onBack} style={{ background: 'none', border: 'none', color: C.slate, fontSize: 13, cursor: 'pointer', fontFamily: 'var(--font-body)', padding: 0 }}>Back to Dashboard</button>
        ...
        {sidebarOpen && (
          <div style={{ width: 250, background: '#111318', overflowY: 'auto', flexShrink: 0, borderRight: '1px solid rgba(255,255,255,0.06)' }}>
            {course.modules.map(mod => (
              <div key={mod.id}>
                <div style={{ padding: '11px 16px', color: C.slate, fontSize: 9, fontFamily: 'var(--font-mono)', letterSpacing: '0.1em', background: 'rgba(255,255,255,0.03)' }}>{mod.title.toUpperCase()}</div>
                  ...
                  return (
                    <div key={lesson.id} onClick={() => { setActiveModule(mod); setActiveLesson(lesson); setView(lesson.type as LMSView); setQuizAnswered(null); setAssignSubmitted(false) }} style={{ padding: '10px 16px', cursor: 'pointer', background: isActive ? 'rgba(243,107,33,0.12)' : 'transparent', borderLeft: `2px solid ${isActive ? accent.primary : 'transparent'}`, display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ color: C.slate, fontSize: 9, fontFamily: 'var(--font-mono)', textTransform: 'uppercase', marginTop: 1 }}>{lesson.type}</div>
                      </div>
                    </div>
                  )
                })}
              ...
              <div>
                <div style={{ background: C.ink, borderRadius: 12, aspectRatio: '16/9', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20, position: 'relative' }}>
                  <div style={{ textAlign: 'center' }}>
                    ...
                    <div style={{ color: C.slate, fontSize: 12 }}>{activeLesson.title} {activeLesson.duration ? `· ${activeLesson.duration}` : ''}</div>
                  </div>
                </div>
            ...
            {view === 'notes' && (
              <div>
                <div style={{ ...glass.panel, borderRadius: 12, padding: 24, marginBottom: 20, color: 'rgba(255,255,255,0.72)', fontSize: 15, lineHeight: 1.85 }}>
                  <p style={{ marginTop: 0 }}>This reading material accompanies the video on <strong style={{ color: C.ink }}>{activeLesson.title}</strong>. Review key concepts, definitions, and formulas that will be tested in the quiz.</p>
                  <div style={{ background: accent.subtle, borderLeft: `3px solid ${accent.primary}`, borderRadius: '0 8px 8px 0', padding: '14px 18px', margin: '16px 0', fontSize: 14 }}>
                    <strong style={{ color: C.ink }}>Key insight:</strong> Structured data analysis transforms raw information into decision-ready insights.
                  </div>
...
function StudentDash({ user, onOpenLMS }: { user: typeof demoUsers[0]; onOpenLMS: () => void }) {
  return (
    <div>
      ...
      <div style={{ background: C.cream, borderRadius: 14, padding: 22, marginBottom: 16 }}>
        <div style={{ color: C.slate, fontSize: 10, fontFamily: 'var(--font-mono)', marginBottom: 8 }}>CONTINUE LEARNING</div>
        <div style={{ color: C.ink, fontSize: 16, fontWeight: 600, marginBottom: 10 }}>Data Analytics — Module 2: Excel</div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
          <div style={{ flex: 1, height: 4, background: T.lineLight, borderRadius: 2 }}><div style={{ width: `${user.progress ?? 0}%`, height: '100%', background: accent.primary, borderRadius: 2 }} /></div>
          <span style={{ color: C.slate, fontSize: 11, fontFamily: 'var(--font-mono)' }}>{user.progress ?? 0}%</span>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }} className="two-col">
        {[['Next Live Class', 'Tomorrow · 10:00 AM', false], ['Pending Assignments', '2 due this week', true], ['Career Readiness', '72% — needs attention', true], ['Certificates Earned', '1 of 5', false]].map(([l, v, warn]) => (
          <div key={l as string} style={{ ...glass.card, border: `1px solid ${warn ? `${accent.primary}33` : T.lineLight}`, borderRadius: 10, padding: '14px 16px' }}>
            <div style={{ color: C.slate, fontSize: 11, marginBottom: 4 }}>{l as string}</div>
            <div style={{ color: warn ? accent.text : C.ink, fontSize: 14, fontWeight: 600 }}>{v as string}</div>
          </div>
        ))}
      </div>
    </div>
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }} className="two-col">
        {[
          ...
          <div key={a.label} style={{ background: a.urgent ? `${accent.primary}10` : C.cream, border: `1px solid ${a.urgent ? `${accent.primary}30` : T.lineDark}`, borderRadius: 10, padding: '14px 18px' }}>
            <div style={{ color: a.urgent ? accent.text : C.muted, fontSize: 9, fontFamily: 'var(--font-mono)', letterSpacing: '0.1em', marginBottom: 5 }}>{a.urgent ? 'NEEDS ATTENTION' : 'UPCOMING'}</div>
            <div style={{ color: C.ink, fontSize: 14, fontWeight: 600, marginBottom: 6 }}>{a.label}</div>
            <button style={{ color: a.urgent ? accent.text : C.slate, background: 'none', border: 'none', fontSize: 12, cursor: 'pointer', fontWeight: 600, padding: 0, fontFamily: 'var(--font-body)' }}>{a.action} →</button>
          </div>
        ))}
      </div>
      ...
      <div style={{ ...glass.panel, borderRadius: 14, padding: 22 }}>
        ...
        {[['Priya M.', 82, false], ['Rahul K.', 65, false], ['Sneha R.', 91, false], ['Amit D.', 48, true]].map(([s, pct, risk]) => (
          <div key={s as string} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '9px 0', borderBottom: `1px solid ${T.lineDark}` }}>
            <div style={{ width: 28, height: 28, borderRadius: '50%', background: risk ? 'rgba(220,38,38,0.15)' : C.cream, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: risk ? C.indigo : C.slate, flexShrink: 0 }}>{(s as string)[0]}</div>
            <div style={{ color: C.ink, fontSize: 13, width: 80, flexShrink: 0 }}>{s as string}</div>
            <div style={{ flex: 1, height: 4, background: C.cream, borderRadius: 2 }}><div style={{ width: `${pct as number}%`, height: '100%', background: (pct as number) < 60 ? '#dc2626' : (pct as number) > 80 ? '#16a34a' : accent.primary, borderRadius: 2 }} /></div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {['42 learners inactive for 7+ days', '18 assignments overdue across Cohort 2024-B', '3 cohorts below 60% completion target'].map(alert => (
            <div key={alert} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ color: C.slate, fontSize: 13 }}>{alert}</span>
      ...
      <div style={{ ...glass.panel, borderRadius: 14, padding: 22 }}>
        ...
        {[['Cohort 2024-A', 91, false], ['Cohort 2024-B', 58, true], ['Cohort 2025-A', 74, false]].map(([c, pct, warn]) => (
          <div key={c as string} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 0', borderBottom: `1px solid ${T.lineDark}` }}>
            ...
            <div style={{ flex: 1, height: 5, background: C.cream, borderRadius: 3 }}><div style={{ width: `${pct as number}%`, height: '100%', background: (pct as number) < 65 ? '#dc2626' : accent.primary, borderRadius: 3 }} /></div>
  ...
  return (
    <div>
      ...
      <div style={{ ...glass.panel, borderRadius: 14, padding: 22, overflowX: 'auto' }}>
        <div style={{ color: C.slate, fontSize: 10, fontFamily: 'var(--font-mono)', marginBottom: 14 }}>RECENT ENROLLMENTS — ILLUSTRATIVE DEMO DATA</div>
          ...
          <tbody>
            {[['Priya S.', 'Data Science & AI', 'Apex College', '₹24,999', 'Active'], ['Rahul K.', 'Full Stack Dev', 'ITM Hyd', '₹29,999', 'Active'], ['Sneha N.', 'Data Analytics', 'Self-enroll', '₹4,999', 'Active']].map((r, i) => (
              <tr key={i} style={{ borderTop: `1px solid ${T.lineDark}` }}>
                {r.map((cell, j) => <td key={j} style={{ padding: '11px 12px 11px 0', color: j === 4 ? C.indigo : C.slate, fontSize: 13 }}>{cell}</td>)}
              </tr>
            ))}
          </tbody>
        </table>
...
export default function OSPage() {
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32, flexWrap: 'wrap', gap: 12 }}>
              <div className="scroll-control-strip" style={{ flex: 1, minWidth: 0 }}>
                ...
                {(['student', 'faculty', 'organisation', 'superadmin'] as Role[]).map(r => (
                  <button key={r} onClick={() => setActiveRole(r)} style={{ padding: '7px 14px', borderRadius: 7, border: `1px solid ${activeRole === r ? accent.primary : T.lineDark}`, background: activeRole === r ? accent.subtle : 'transparent', color: activeRole === r ? accent.text : C.slate, fontSize: 11, cursor: 'pointer', fontFamily: 'var(--font-mono)', textTransform: 'capitalize', transition: 'all 0.2s', flexShrink: 0, whiteSpace: 'nowrap' }}>{r === 'superadmin' ? 'Super Admin' : r.charAt(0).toUpperCase() + r.slice(1)}</button>
                ))}
                </div>
              </div>
              <button onClick={() => setActiveRole(null)} style={{ background: 'none', border: 'none', color: C.slate, fontSize: 13, cursor: 'pointer', fontFamily: 'var(--font-body)', flexShrink: 0 }}>← Back to login</button>
            </div>
  ...
  return (
    <PageShell aurora={false}>
      <section className="os-gateway" style={{ background: C.canvas, minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '100px 32px 80px', position: 'relative' }}>
        <div style={{ maxWidth: 920, margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
          <FadeIn>
            <div style={{ marginBottom: 56, maxWidth: 640 }}>
              <div className="skylent-label" style={{ color: C.indigo, marginBottom: 20 }}>Skylent OS — demo environment</div>
              <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(36px, 5vw, 58px)', fontWeight: 600, color: C.ink, letterSpacing: '-0.03em', lineHeight: 1.05, margin: '0 0 16px' }}>
                Enter the operating layer<br /><em style={{ fontStyle: 'italic', fontWeight: 500 }}>behind Skylent.</em>
              </h1>
              <p style={{ color: C.slate, fontSize: 16, lineHeight: 1.75, maxWidth: 460, margin: 0 }}>A learning and career workspace. Select a role to explore the product — demo only, no live accounts.</p>
            </div>
          </FadeIn>

          <div style={{ borderTop: `1px solid ${T.lineStrong}` }}>
            {demoUsers.map((u, i) => (
              <FadeIn key={u.role} delay={i * 40}>
                <button
                  type="button"
                  onClick={() => setActiveRole(u.role as Role)}
                  style={{
                    display: 'grid',
                ...
                >
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: C.indigo }}>0{i + 1}</div>
                  <div>
                    <div style={{ color: C.ink, fontSize: 18, fontWeight: 600, fontFamily: 'var(--font-display)', letterSpacing: '-0.02em' }}>{u.name}</div>
                    <div style={{ color: C.slate, fontSize: 13, marginTop: 4 }}>
                      ...
                      {u.role === 'faculty' && ` · ${u.course}`}
                      {u.role === 'organisation' && ` · ${u.programs} programmes`}
                    </div>
                  </div>
                  <div style={{ color: C.indigo, fontSize: 13, fontWeight: 600, whiteSpace: 'nowrap' }}>Enter →</div>
                </button>
              </FadeIn>
            ))}
          </div>
          ...
          <FadeIn delay={240}>
            <div style={{ marginTop: 28, color: C.slate, fontSize: 12, fontFamily: 'var(--font-mono)' }}>
              Demo only — no real accounts, payments, or operating data.
            </div>
          </FadeIn>
        </div>
      </section>
