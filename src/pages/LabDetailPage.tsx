...
const statusColors: Record<LabExperimentStatus, { color: string; label: string }> = {
  ...
  submitted: { color: C.blue, label: 'Submitted' },
  completed: { color: C.indigo, label: 'Completed' },
}
  ...
  return (
    <div className="skylent-lab-shell" style={{ minHeight: '100vh', background: C.canvas, fontFamily: 'var(--font-body)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', flex: 1 }}>
        {/* Left sidebar */}
        <div style={{ width: 240, flexShrink: 0, background: C.cream, borderRight: '1px solid rgba(21,23,26,0.10)', overflowY: 'auto', position: 'sticky', top: 52, height: 'calc(100vh - 52px)' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '48px 48px 80px' }}>
          ...
          <div style={{ marginBottom: 48 }}>
            <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
              <span style={{ background: C.cream, color: C.slate, fontSize: 11, fontFamily: 'var(--font-mono)', padding: '4px 12px', borderRadius: 100, border: '1px solid rgba(255,255,255,0.1)' }}>{subject.program}</span>
              <span style={{ background: C.cream, color: C.slate, fontSize: 11, fontFamily: 'var(--font-mono)', padding: '4px 12px', borderRadius: 100, border: '1px solid rgba(255,255,255,0.08)' }}>{subject.semester}</span>
            </div>
          ...
          <div style={{ marginBottom: 48 }}>
            ...
            <div style={{ background: C.cream, borderRadius: 4, height: 6 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {subject.experiments.map(exp => {
              ...
              return (
                <div
                  key={exp.id}
                  style={{ background: C.cream, border: '1px solid rgba(21,23,26,0.10)', borderRadius: 0, padding: '24px 28px', display: 'flex', alignItems: 'flex-start', gap: 20, transition: 'border-color 0.15s' }}
                  onMouseEnter={e => (e.currentTarget.style.borderColor = 'rgba(21,23,26,0.22)')}
                  onMouseLeave={e => (e.currentTarget.style.borderColor = 'rgba(21,23,26,0.10)')}
                >
                  {/* Number */}
                  <div style={{ width: 44, height: 44, borderRadius: 10, background: C.canvas, border: '1px solid rgba(21,23,26,0.10)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
