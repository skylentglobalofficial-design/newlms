...
const statusColors: Record<LabExperimentStatus, { color: string; label: string }> = {
  not_started: { color: C.slate, label: 'Not Started' },
...
export default function LabDetailPage() {
  ...
  if (!subject) {
    return (
      <div style={{ minHeight: '100vh', background: C.ink, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, fontFamily: 'var(--font-body)' }}>
        <div style={{ color: C.ink, fontSize: 24, fontFamily: 'var(--font-display)', fontWeight: 700 }}>Lab not found</div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0c0e', fontFamily: 'var(--font-body)', display: 'flex', flexDirection: 'column' }}>
      {/* Top bar */}
      <div style={{ height: 52, background: C.ink, borderBottom: '1px solid C.slate', display: 'flex', alignItems: 'center', padding: '0 28px', gap: 16, flexShrink: 0, position: 'sticky', top: 0, zIndex: 50 }}>
        ...
        <div style={{ marginLeft: 'auto', color: C.slate, fontSize: 12, fontFamily: 'var(--font-mono)', flexShrink: 0 }}>
      ...
      <div style={{ display: 'flex', flex: 1 }}>
        {/* Left sidebar */}
        <div style={{ width: 240, flexShrink: 0, background: C.ink, borderRight: '1px solid C.slate', overflowY: 'auto', position: 'sticky', top: 52, height: 'calc(100vh - 52px)' }}>
          <div style={{ padding: '16px 16px 8px', color: C.slate, fontSize: 9, fontFamily: 'var(--font-mono)', letterSpacing: '0.12em' }}>EXPERIMENTS</div>
          {subject.experiments.map(exp => {
            ...
            return (
              <button
                ...
                onMouseEnter={e => (e.currentTarget.style.background = 'rgba(11,13,15,0.04)')}
                onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
              >
                {/* Status dot */}
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: sc.color, flexShrink: 0, marginTop: 4, border: status === 'not_started' ? '1px solid C.slate' : 'none', boxSizing: 'border-box' }} />
                <div style={{ flex: 1, overflow: 'hidden' }}>
                  <div style={{ color: C.slate, fontSize: 10, fontFamily: 'var(--font-mono)', marginBottom: 2 }}>Exp {exp.number}</div>
                  <div style={{ color: C.slate, fontSize: 12, lineHeight: 1.4, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{exp.title}</div>
                </div>
              </button>
            )
          })}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '48px 48px 80px' }}>
          ...
          <div style={{ marginBottom: 48 }}>
            <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
              <span style={{ background: '$rgba(11,13,15,0.04)', color: C.slate, fontSize: 11, fontFamily: 'var(--font-mono)', padding: '4px 12px', borderRadius: 100, border: `1px solid ${T.lineLight}` }}>{subject.program}</span>
              <span style={{ background: '#FFFDFC', color: C.slate, fontSize: 11, fontFamily: 'var(--font-mono)', padding: '4px 12px', borderRadius: 100, border: `1px solid ${T.lineLight}` }}>{subject.semester}</span>
            </div>
            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 36, fontWeight: 700, color: C.ink, lineHeight: 1.1, margin: '0 0 14px', letterSpacing: '-0.025em' }}>{subject.subject}</h1>
            <p style={{ color: C.slate, fontSize: 15, lineHeight: 1.7, maxWidth: 580, margin: '0 0 24px' }}>{subject.desc}</p>
            <div style={{ display: 'flex', gap: 24, alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <span style={{ color: C.slate, fontSize: 13, fontFamily: 'var(--font-mono)' }}>{subject.experiments.length} Experiments</span>
              <span style={{ color: C.slate, fontSize: 13, fontFamily: 'var(--font-mono)' }}>{completedCount} Completed</span>
            </div>
          </div>
          ...
          <div style={{ marginBottom: 48 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
              <span style={{ color: C.slate, fontSize: 12, fontFamily: 'var(--font-mono)' }}>Overall Progress</span>
            ...
            <div style={{ background: 'rgba(11,13,15,0.08)', borderRadius: 4, height: 6 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {subject.experiments.map(exp => {
              ...
              return (
                <div
                  key={exp.id}
                  style={{ background: 'rgba(11,13,15,0.03)', border: `1px solid ${T.lineLight}`, borderRadius: 12, padding: '24px 28px', display: 'flex', alignItems: 'flex-start', gap: 20, transition: 'border-color 0.15s' }}
                  onMouseEnter={e => (e.currentTarget.style.borderColor = T.lineLight)}
                  onMouseLeave={e => (e.currentTarget.style.borderColor = T.lineLight)}
                >
                  {/* Number */}
                  <div style={{ width: 44, height: 44, borderRadius: 10, background: '$rgba(11,13,15,0.04)', border: `1px solid ${T.lineLight}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <span style={{ color: C.slate, fontSize: 14, fontFamily: 'var(--font-mono)', fontWeight: 600 }}>{String(exp.number).padStart(2, '0')}</span>
                  </div>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', gap: 8, marginBottom: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                      ...
                      <span style={{ color: C.slate, fontSize: 11, fontFamily: 'var(--font-mono)' }}>{exp.duration}</span>
                    </div>
                    <div style={{ color: C.ink, fontSize: 16, fontWeight: 600, marginBottom: 6 }}>{exp.title}</div>
                    <div style={{ color: C.slate, fontSize: 13, lineHeight: 1.6 }}>{exp.objective}</div>
                  </div>
