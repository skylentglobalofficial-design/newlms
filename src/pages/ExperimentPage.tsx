  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ background: 'rgba(79,70,229,0.08)', color: '#4F46E5', border: '1px solid rgba(79,70,229,0.28)', fontSize: 11, fontFamily: 'var(--font-mono)', padding: '4px 12px', borderRadius: 100 }}>Python 3.11</div>
      </div>

      {/* Editor */}
      <div style={{ background: C.cream, border: '1px solid rgba(21,23,26,0.10)', borderRadius: 10, overflow: 'hidden' }}>
        <div style={{ background: C.canvas, padding: '10px 16px', borderBottom: '1px solid rgba(21,23,26,0.10)', display: 'flex', gap: 8, alignItems: 'center' }}>
          <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#C4B8A8' }} />
          <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#C4B8A8' }} />
          <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#C4B8A8' }} />
        ...
        <div style={{ padding: '16px', fontFamily: 'var(--font-mono)', fontSize: 13, lineHeight: 1.8 }}>
          {codeLines.map((line, i) => (
            <div key={i} style={{ display: 'flex', gap: 16 }}>
              ...
              <span style={{ color: line.startsWith('#') ? C.muted : line.startsWith('import') || line.startsWith('from') ? C.indigo : line.includes('=') ? C.blue : C.ink }}>
                {line || ' '}
              </span>
            </div>
          ))}
      ...
      <button
        onClick={() => setRan(true)}
        style={{ alignSelf: 'flex-start', background: C.indigo, border: 'none', color: C.white, padding: '10px 20px', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--font-body)', display: 'flex', alignItems: 'center', gap: 8 }}
      >
      ...
      {ran && (
        <div style={{ background: C.cream, border: '1px solid rgba(34,197,94,0.2)', borderRadius: 10, overflow: 'hidden' }}>
          <div style={{ background: C.canvas, padding: '8px 16px', borderBottom: '1px solid rgba(21,23,26,0.10)', display: 'flex', alignItems: 'center', gap: 8 }}>
...
function DataWorkspace({ title }: { title: string }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {cells.map((cell, i) => (
        <div key={i} style={{ border: '1px solid rgba(21,23,26,0.10)', borderRadius: 10, overflow: 'hidden' }}>
          {cell.type === 'markdown' ? (
            ...
            <div>
              <div style={{ background: C.cream, padding: '14px 20px', fontFamily: 'var(--font-mono)', fontSize: 12, lineHeight: 1.8, color: C.slate }}>
                {cell.content.split('\n').map((l, li) => (
                  <div key={li} style={{ color: l.startsWith('#') ? C.muted : l.startsWith('import') || l.startsWith('from') ? C.indigo : C.ink }}>{l || ' '}</div>
                ))}
              </div>
              <div style={{ background: C.canvas, padding: '8px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                <button
                  onClick={() => setExecuted(prev => ({ ...prev, [i]: true }))}
                  style={{ background: 'none', border: '1px solid rgba(255,255,255,0.12)', color: C.slate, padding: '4px 12px', borderRadius: 6, fontSize: 11, cursor: 'pointer', fontFamily: 'var(--font-mono)' }}
                >Execute Cell</button>
                {executed[i] && <span style={{ color: '#4ade80', fontSize: 11, fontFamily: 'var(--font-mono)' }}>Done</span>}
              </div>
              {executed[i] && (
                <div style={{ background: C.cream, padding: '12px 20px', borderTop: '1px solid rgba(255,255,255,0.05)', fontFamily: 'var(--font-mono)', fontSize: 11, color: C.slate, lineHeight: 1.8 }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        ...
        <div style={{ background: C.cream, border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, padding: '20px 24px' }}>
      ...
      {[
        ...
        <div key={field.label}>
          ...
          <textarea
            ...
            rows={4}
            style={{ width: '100%', background: C.cream, border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '12px 16px', color: C.ink, fontSize: 13, fontFamily: 'var(--font-body)', lineHeight: 1.7, resize: 'vertical', outline: 'none', boxSizing: 'border-box' }}
          />
        </div>
      ))}
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        <button
          ...
          style={{ background: C.cream, border: '1px solid rgba(255,255,255,0.12)', color: saved ? C.indigo : C.ink, padding: '10px 20px', borderRadius: 8, fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'var(--font-body)' }}
...
function SimulationWorkspace({ title, instructions }: { title: string; instructions: string[] }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <button
        onClick={() => setRan(true)}
        style={{ alignSelf: 'flex-start', background: C.indigo, border: '1px solid rgba(79,70,229,0.35)', color: C.white, padding: '10px 24px', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--font-body)' }}
        ...
        <div style={{ background: 'rgba(168,85,247,0.06)', border: '1px solid rgba(168,85,247,0.2)', borderRadius: 10, padding: '20px 24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
            {[
              ...
              <div key={m.label} style={{ background: C.cream, border: '1px solid rgba(255,255,255,0.07)', borderRadius: 8, padding: '14px' }}>
...
export default function ExperimentPage() {
  ...
  return (
    <div className="skylent-lab-shell" style={{ minHeight: '100vh', background: C.canvas, fontFamily: 'var(--font-body)', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Top bar */}
      <div style={{ height: 48, background: C.cream, borderBottom: '1px solid rgba(21,23,26,0.10)', display: 'flex', alignItems: 'center', padding: '0 20px', gap: 12, flexShrink: 0, position: 'sticky', top: 0, zIndex: 50 }}>
      ...
      <div style={{ display: 'flex', flex: 1, height: 'calc(100vh - 48px)', overflow: 'hidden' }}>
        ...
        <div style={{ width: 200, flexShrink: 0, background: C.cream, borderRight: '1px solid rgba(21,23,26,0.10)', overflowY: 'auto' }}>
          ...
          {subject.experiments.map(exp => {
            ...
            return (
              <button
                ...
                style={{ display: 'flex', width: '100%', textAlign: 'left', padding: '10px 14px', gap: 10, alignItems: 'flex-start', background: 'transparent', borderLeft: isActive ? `3px solid ${C.indigo}` : '3px solid transparent', border: 'none', cursor: 'pointer', transition: 'all 0.15s', borderRight: 'none' }}
                onMouseEnter={e => { if (!isActive) (e.currentTarget as HTMLButtonElement).style.background = C.cream }}
        ...
        <div style={{ width: 280, flexShrink: 0, background: C.cream, borderLeft: '1px solid rgba(21,23,26,0.10)', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
          <div style={{ padding: '20px 20px 0' }}>
            ...
            {/* Status */}
            <div style={{ background: C.cream, border: '1px solid rgba(255,255,255,0.06)', borderRadius: 8, padding: '10px 14px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {experiment.tasks.map((task, i) => {
                  ...
                  return (
                    ...
                    >
                      <div style={{ width: 16, height: 16, borderRadius: 4, border: `1.5px solid ${checked ? C.indigo : 'rgba(21,23,26,0.18)'}`, background: checked ? 'rgba(79,70,229,0.12)' : 'transparent', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 1 }}>
                        {checked && <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke={C.indigo} strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>}
                      </div>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ color: C.slate, fontSize: 11.5, lineHeight: 1.7, background: C.cream, border: '1px solid rgba(255,255,255,0.05)', borderRadius: 8, padding: '12px' }}>
            ...
            {(currentStatus === 'not_started' || currentStatus === 'in_progress') && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <button
                  ...
                  style={{ width: '100%', background: C.indigo, border: 'none', color: C.white, padding: '11px', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--font-body)', transition: 'opacity 0.15s' }}
            ...
            {currentStatus === 'submitted' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <button
                  ...
                  style={{ width: '100%', background: C.cream, border: '1px solid rgba(255,255,255,0.1)', color: C.slate, padding: '10px', borderRadius: 8, fontSize: 12, cursor: 'pointer', fontFamily: 'var(--font-body)' }}
