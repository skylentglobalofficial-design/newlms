    ...
    <GlassSurface level={2} padding="0" style={{ overflow: 'hidden', position: 'relative' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, padding: 'clamp(24px, 3.5vw, 36px)' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.52)', fontSize: 14, lineHeight: 1.6, margin: '0 0 24px', maxWidth: 640 }}>
          The Super Admin workspace is connected to the authenticated platform role. Live platform metrics are intentionally shown only when backed by the admin API.
        </p>
...
function AdminSection({ id, title, description }: { id: string; title: string; description: string }) {
  return (
    <div id={id} style={canvasRow}>
      <div style={{ color: 'rgba(255,255,255,0.48)', fontSize: 11, marginBottom: 12 }}>{title}</div>
      <RoleSectionEmpty title={`${title} API not connected`} description={description} />
    </div>
  )
}
    ...
    >
      <div id="admin-overview">
        <RoleWorkspaceBanner
          ...
          description="Live platform data is displayed only when backed by connected admin APIs. This workspace does not invent operational metrics or business activity."
        />
        <AdminOverview />
        ...
        <AdminSection id="admin-system" title="System" description="Service health and uptime must come from live monitoring rather than placeholder values." />
      </div>
    </AuthDashboardShell>
  )
}