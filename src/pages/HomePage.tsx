...
function ProductPreview() {
  ...
  return (
    <div className="home-product-preview" aria-label="Skylent learning workspace preview">
      <div className="preview-topbar"><span className="preview-dot" /><span>SKYLENT / LEARNING STUDIO</span><span className="preview-status">LIVE PRACTICE</span></div>
      <div className="preview-body">
        <div className="preview-sidebar"><div className="preview-sidebar-title">TODAY</div>{['Brief', 'Investigation', 'Build', 'Feedback'].map((item, index) => <div key={item} className={`preview-side-item ${index === 1 ? 'is-active' : ''}`}><b>0{index + 1}</b>{item}</div>)}</div>
        <div className="preview-workspace"><div className="preview-kicker">DATA INVESTIGATION / 24 MIN LEFT</div><h3>Where should this small business grow next?</h3><p>Explore the customer data, find a pattern, and make one defensible recommendation.</p><div className="preview-chart" aria-hidden="true"><div className="chart-grid" />{[42, 66, 49, 78, 60, 86, 72].map((height, index) => <span key={index} style={{ height: `${height}%` }} />)}</div><div className="preview-workspace-footer"><span>Evidence saved automatically</span><button type="button" onClick={() => setActive((active + 1) % intents.length)}>Try next step <span>→</span></button></div></div>
      </div>
  ...
  return (
    <PageShell aurora={false}>
      <main className="home-redesign">
        ...
        <section id="interests" className="home-section home-interest-section"><div className="home-section-heading"><div><SectionLabel>Start where you are</SectionLabel><h2>Different worlds.<br /><em>One platform.</em></h2></div><p>Choose a direction, then move between understanding, practice, projects and opportunities as your goals evolve.</p></div><div className="home-domain-grid">{domains.map((domain, index) => <Link to={domain.to} className="home-domain-card" key={domain.label} style={{ '--domain-accent': domain.accent } as React.CSSProperties}><MediaImage src={domain.image} alt="" aspect="3/2" radius={0} /><div className="home-domain-content"><span className="home-domain-number">0{index + 1}</span><h3>{domain.label}</h3><p>{domain.copy}</p><span className="home-card-arrow">Explore <b>→</b></span></div></Link>)}</div></section>

        <section className="home-section home-engine-section"><div className="home-section-heading"><div><SectionLabel>The learning engine</SectionLabel><h2>The medium should<br /><em>fit the objective.</em></h2></div><p>A lesson is useful when it matches what the learner is trying to do. Sometimes that means explanation. Sometimes it means a case, a simulation, a build or a hard question.</p></div><div className="home-engine-grid"><div className="home-engine-list">{['Simulation', 'Case', 'Build', 'Practice'].map((item, index) => <button type="button" key={item} className={`home-engine-item ${intent === index ? 'is-active' : ''}`} onClick={() => setIntent(index)}><span>0{index + 1}</span><strong>{item}</strong><i>↗</i></button>)}<div className="home-engine-note"><strong>{selectedIntent.label}</strong><span>{selectedIntent.copy}</span></div></div><div className="home-engine-workspace"><div className="workspace-heading"><span>ACTIVE WORKSPACE</span><b>02 / 04</b></div><h3>{intent === 0 ? 'Test the idea before you memorise it.' : intent === 1 ? 'Make a decision. See what changes.' : intent === 2 ? 'Build a useful thing from the brief.' : 'Practice with feedback that tells you why.'}</h3><div className="workspace-lines"><span /><span /><span /></div><div className="workspace-action"><span>Learning objective</span><strong>{selectedIntent.copy}</strong><button type="button" onClick={() => setIntent((intent + 1) % intents.length)}>Continue <span>→</span></button></div></div></div></section>
