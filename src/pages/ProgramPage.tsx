  ...
  return (
    <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ flexShrink: 0, marginTop: 2 }}>
        <rect width="16" height="16" rx="4" fill="rgba(243,107,33,0.15)" stroke="rgba(243,107,33,0.3)" strokeWidth="0.8" />
...
function StickyProgramNav({
  sections, activeId, ctaLabel, onCTA,
}: {
  sections: NavSection[]
  activeId: string
  ctaLabel: string
          ...
          {sections.map(s => (
            <button
              ...
              style={{
                ...
                padding: '14px 14px', color: activeId === s.id ? C.orange : 'rgba(255,255,255,0.42)',
    ...
    <PageShell auroraTheme={auroraTheme}>
      ...
      <section id="program-hero" style={{ position: 'relative', overflow: 'hidden', padding: `${T.navH + 32}px ${T.gutter} 0` }}>
        ...
        <div style={{ maxWidth: T.maxW, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 'clamp(32px,5vw,56px)', alignItems: 'start' }} className="program-detail-grid">
            <div>
              ...
              {program.whatYouWillLearn && program.whatYouWillLearn.length > 0 && (
                <FadeIn delay={100}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px 28px', marginTop: 32 }} className="two-col-sm">
          ...
          {/* Quick facts strip */}
          <div style={{ paddingTop: 32, paddingBottom: 36, marginTop: 8, borderTop: `1px solid ${T.lineDark}`, display: 'flex', gap: 'clamp(20px,4vw,48px)', flexWrap: 'wrap' }}>
            {[
      ...
      <StickyProgramNav sections={navSections} activeId={activeSection} ctaLabel={ctaLabel} onCTA={() => setApplyOpen(true)} />

      {/* ── OVERVIEW ──────────────────────────────────────────────────────── */}
      <Section id="overview" tone="canvas" divider>
        <FadeIn>
          <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 0.9fr', gap: 'clamp(32px,5vw,64px)', alignItems: 'start', marginBottom: 56 }} className="two-col program-overview-split">
            <div>
        ...
        {program.whatYouWillLearn && program.whatYouWillLearn.length > 0 && (
          <FadeIn>
            ...
            />
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '14px 36px', marginTop: 36 }} className="two-col-sm">
        ...
        {program.whoIsItFor && program.whoIsItFor.length > 0 && (
          <FadeIn>
            <div style={{ marginTop: 64, display: 'grid', gridTemplateColumns: '240px 1fr', gap: 'clamp(24px,4vw,56px)', alignItems: 'start' }} className="program-who-split">
              <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                {program.whoIsItFor.map((who, i) => (
                  <div key={i} style={{ display: 'flex', gap: 16, alignItems: 'flex-start', padding: '16px 0', borderBottom: i < program.whoIsItFor!.length - 1 ? `1px solid ${T.lineDark}` : 'none' }}>
                    <div style={{ width: 6, height: 6, borderRadius: '50%', background: C.orange, flexShrink: 0, marginTop: 7 }} />
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '48px 1fr', gap: '0 24px', position: 'relative' }} className="program-curriculum-row">
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                      <div style={{
                        ...
                        background: isOpen ? 'rgba(243,107,33,0.12)' : 'rgba(255,255,255,0.04)',
                        border: `1px solid ${isOpen ? 'rgba(243,107,33,0.35)' : T.lineDark}`,
                        fontFamily: 'var(--font-mono)', fontSize: 11, color: isOpen ? C.orange : 'rgba(255,255,255,0.4)',
                      }}>
                        {mod.number}
                      </div>
                    ...
                    <div style={{ paddingBottom: isLast ? 0 : 28, borderBottom: isLast ? 'none' : `1px solid ${T.lineDark}` }}>
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0, paddingTop: 4 }}>
                          ...
                          <span style={{ color: C.orange, fontSize: 18, transform: isOpen ? 'rotate(45deg)' : 'none', transition: 'transform 0.2s' }}>+</span>
                        </div>
                      </button>
                      {isOpen && (
      ...
      {program.projectsDetail && program.projectsDetail.length > 0 && (
        <Section id="projects" tone="canvas" divider>
          ...
          {program.projectsDetail[featuredProject] && (() => {
            ...
            return (
              <FadeIn delay={40}>
                <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 0.9fr', gap: 'clamp(28px,4vw,48px)', alignItems: 'center', marginTop: 44 }} className="two-col program-project-featured">
                  <GlassSurface level={3} padding="24px 26px">
                    <div className="skylent-label" style={{ color: 'rgba(255,255,255,0.28)', marginBottom: 16 }}>Project artifact</div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
                      <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 8, padding: 14, minHeight: 80 }}>
                        <div style={{ color: 'rgba(255,255,255,0.25)', fontSize: 9, fontFamily: 'var(--font-mono)', marginBottom: 8 }}>DATA</div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                          {[70, 55, 80].map((w, i) => (
                            <div key={i} style={{ height: 5, width: `${w}%`, background: 'rgba(255,255,255,0.1)', borderRadius: 2 }} />
                          ))}
                        </div>
                      </div>
                      <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 8, padding: 14, minHeight: 80 }}>
                        <div style={{ color: 'rgba(255,255,255,0.25)', fontSize: 9, fontFamily: 'var(--font-mono)', marginBottom: 8 }}>RESULT</div>
                        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 3, height: 48 }}>
                          {[35, 55, 45, 70, 50, 65].map((h, i) => (
                            <div key={i} style={{ flex: 1, height: `${h}%`, background: i === 3 ? C.orange : 'rgba(255,255,255,0.12)', borderRadius: 2 }} />
                          ))}
                        </div>
                      </div>
                    </div>
          ...
          {program.projectsDetail.length > 1 && (
            <div style={{ marginTop: 40, display: 'flex', flexDirection: 'column', gap: 0 }}>
              {program.projectsDetail.map((proj, i) => (
                ...
                >
                  <div>
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: featuredProject === i ? C.orange : 'rgba(255,255,255,0.3)' }}>
          ...
          <FadeIn>
            ...
            <div style={{ marginTop: 36, display: 'flex', flexDirection: 'column', gap: 0 }}>
              {program.learningExperience.map((item, i) => (
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '40px 1fr', gap: 20, padding: '20px 0', borderBottom: i < program.learningExperience!.length - 1 ? `1px solid ${T.lineDark}` : 'none' }}>
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: C.orange, paddingTop: 2 }}>{String(i + 1).padStart(2, '0')}</div>
      ...
      {/* ── REVIEWS ───────────────────────────────────────────────────────── */}
      {!isExamPrep && (
        <Section id="reviews" tone="canvas" divider>
          <FadeIn>
            <SectionHeader
              tone="dark"
              eyebrow="Learner outcomes"
              title="Sample learner stories"
              lead="Verified stories will appear here when published."
            />
          </FadeIn>
          <div style={{ marginTop: 40, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 'clamp(24px,4vw,40px)' }} className="three-col program-reviews-grid">
            {stories.slice(0, 3).map((s, i) => (
              <FadeIn key={s.name} delay={i * 60}>
                <div style={{ paddingRight: 8 }}>
                  <div style={{ fontFamily: 'var(--font-display)', fontSize: 32, color: C.orange, lineHeight: 1, marginBottom: 16, opacity: 0.5 }}>"</div>
                  <p style={{ color: 'rgba(255,255,255,0.62)', fontSize: 14, lineHeight: 1.72, margin: '0 0 24px' }}>{s.provided}</p>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, paddingTop: 18, borderTop: `1px solid ${T.lineDark}` }}>
                    <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'rgba(243,107,33,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.orange, fontSize: 12, fontWeight: 700 }}>{s.initials}</div>
                    <div>
                      <div style={{ fontSize: 14, color: C.white, fontWeight: 600 }}>{s.name}</div>
                      <div className="skylent-label" style={{ color: C.orange, marginTop: 2, fontSize: 9 }}>{s.outcome}</div>
                    </div>
                  </div>
                </div>
              </FadeIn>
      ...
      {/* ── CERTIFICATION ─────────────────────────────────────────────────── */}
      <Section tone="canvas" divider style={{ paddingTop: T.sectionSm, paddingBottom: T.sectionSm }}>
        <FadeIn>
          <div style={{ display: 'flex', gap: 24, alignItems: 'center', flexWrap: 'wrap' }}>
            <div style={{ width: 48, height: 48, borderRadius: 12, background: `linear-gradient(135deg, ${C.orange}, #ff9a3c)`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2"><circle cx="12" cy="8" r="6"/><path d="M8.5 14.5L6 22l6-2 6 2-2.5-7.5"/></svg>
            </div>
            <div style={{ flex: 1, minWidth: 200 }}>
              <div className="skylent-label" style={{ color: 'rgba(255,255,255,0.3)', marginBottom: 4 }}>Certificate</div>
              <div style={{ fontFamily: 'var(--font-display)', fontSize: 18, fontWeight: 600, color: C.white, marginBottom: 4 }}>{program.cert}</div>
              <div style={{ color: 'rgba(255,255,255,0.42)', fontSize: 14 }}>
                Issued by Skylent on successful completion. Includes a verifiable credential ID.
      ...
      {program.faqs && program.faqs.length > 0 && (
        <Section id="faq" tone="canvas" divider>
          <FadeIn>
                ...
                <FadeIn key={i} delay={i * 30}>
                  <div style={{ borderBottom: `1px solid ${T.lineDark}` }}>
                    ...
                    >
                      ...
                      <span style={{ color: C.orange, fontSize: 20, transform: isOpen ? 'rotate(45deg)' : 'none', transition: 'transform 0.2s', flexShrink: 0 }}>+</span>
                    </button>
                    {isOpen && (
      ...
      {/* ── PRICING COMPARISON ────────────────────────────────────────────── */}
      <Section id="pricing" tone="canvas" divider>
        <FadeIn>
          <SectionHeader
            tone="dark"
        ...
        <div style={{ marginTop: 44, overflowX: 'auto' }} className="program-pricing-wrap">
          <div style={{ minWidth: 560 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: `1.4fr repeat(${program.pricing.length}, 1fr)`, gap: 0, borderBottom: `1px solid ${T.lineDark}` }}>
              ...
              {program.pricing.map(tier => (
                <div key={tier.name} style={{ padding: '16px 20px', textAlign: 'center', borderLeft: `1px solid ${T.lineDark}`, background: tier.highlight ? 'rgba(243,107,33,0.06)' : 'transparent' }}>
                  <div style={{ fontFamily: 'var(--font-display)', fontSize: 15, fontWeight: 600, color: tier.highlight ? C.orange : C.white }}>{tier.name}</div>
                  {tier.highlight && <div className="skylent-label" style={{ color: C.orange, fontSize: 9, marginTop: 4 }}>Recommended</div>}
                </div>
              ))}
            </div>
            {/* Prices */}
            <div style={{ display: 'grid', gridTemplateColumns: `1.4fr repeat(${program.pricing.length}, 1fr)`, gap: 0, borderBottom: `1px solid ${T.lineDark}` }}>
              ...
              {program.pricing.map(tier => (
                <div key={tier.name} style={{ padding: '20px', textAlign: 'center', borderLeft: `1px solid ${T.lineDark}`, background: tier.highlight ? 'rgba(243,107,33,0.04)' : 'transparent' }}>
            ...
            {allPricingFeatures.map((feature, fi) => (
              <div key={feature} style={{ display: 'grid', gridTemplateColumns: `1.4fr repeat(${program.pricing.length}, 1fr)`, gap: 0, borderBottom: `1px solid ${T.lineDark}` }}>
                ...
                {program.pricing.map(tier => (
                  <div key={tier.name} style={{ padding: '14px 20px', textAlign: 'center', borderLeft: `1px solid ${T.lineDark}`, background: tier.highlight ? 'rgba(243,107,33,0.03)' : 'transparent' }}>
                    {tier.features.includes(feature) ? (
                      <span style={{ color: tier.highlight ? C.orange : 'rgba(255,255,255,0.5)', fontSize: 14 }}>✓</span>
                    ) : (
      ...
      {/* ── FINAL CTA ─────────────────────────────────────────────────────── */}
      <section style={{ position: 'relative', overflow: 'hidden', padding: `${T.section} ${T.gutter}` }}>
        <Aurora themeId={auroraTheme} />
