...
function Panel({ label, accent, children, highlight }: { label: string; accent: Accent; children: ReactNode; highlight?: boolean }) {
  return (
    <div style={{
      background: highlight ? accent.subtle : C.cream,
      border: `1px solid ${highlight ? accent.border : T.lineDark}`,
      borderRadius: 10, padding: '12px 12px', minHeight: 100,
    }}>
      <div style={{ color: highlight ? accent.textMuted : C.slate, fontSize: 9, fontFamily: 'var(--font-mono)', marginBottom: 10, letterSpacing: '0.08em' }}>{label}</div>
      {children}
    </div>
  )
}
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 0.9fr 1fr', gap: 10 }} className="program-workflow-panels">
      <Panel label="DATASET" accent={accent}>
        ...
        {rows.map((row, ri) => (
          <div key={ri} style={{ display: 'grid', gridTemplateColumns: `repeat(${cols.length}, 1fr)`, gap: 4, padding: '3px 0' }}>
            {row.map((cell, ci) => <div key={ci} style={{ fontSize: 9, fontFamily: 'var(--font-mono)', color: cell === '1' && ci === 2 ? accent.text : C.slate }}>{cell}</div>)}
          </div>
        ))}
      </Panel>
      <Panel label="METRICS" accent={accent}>
        <div style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: C.slate, marginBottom: 10 }}>experiment · run 12</div>
        {[{ label: 'accuracy', w: 82 }, { label: 'f1', w: 76 }, { label: 'auc', w: 71 }].map(m => (
          <div key={m.label} style={{ marginBottom: 6 }}>
            <div style={{ fontSize: 8, fontFamily: 'var(--font-mono)', color: C.slate, marginBottom: 3 }}>{m.label}</div>
      ...
      <Panel label="PROJECT" accent={accent} highlight>
        <div style={{ fontSize: 11, color: C.ink, lineHeight: 1.5, marginBottom: 8 }}>Churn prediction notebook</div>
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }} className="program-workflow-panels">
      <Panel label="CODE" accent={accent}>
        <div style={{ fontSize: 9, fontFamily: 'var(--font-mono)', color: C.slate, lineHeight: 1.7 }}>
      ...
      <Panel label="API" accent={accent}>
        ...
        <div style={{ fontSize: 9, fontFamily: 'var(--font-mono)', color: C.slate, marginTop: 8 }}>200 · 24ms</div>
      </Panel>
      <Panel label="DATABASE" accent={accent}>
        <div style={{ fontSize: 9, fontFamily: 'var(--font-mono)', color: C.slate, lineHeight: 1.6 }}>
          users (id, email, role)<br />sessions (id, user_id)
        </div>
      </Panel>
      <Panel label="LOGS" accent={accent} highlight>
        <div style={{ fontSize: 8, fontFamily: 'var(--font-mono)', color: C.slate, lineHeight: 1.7 }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 10, flexWrap: 'wrap' }}>
          {['Physics', 'Chemistry', 'Math'].map((s, i) => (
            <span key={s} style={{
              ...
              background: i === 2 ? mathAccent.subtle : C.cream,
              border: `1px solid ${i === 2 ? mathAccent.border : T.lineDark}`,
              color: i === 2 ? mathAccent.text : C.slate,
            }}>{s}</span>
          ))}
        </div>
      ...
      <Panel label="CASE · DILR" accent={accent}>
        <div style={{ display: 'flex', gap: 6, marginBottom: 10, flexWrap: 'wrap' }}>
          {['VARC', 'DILR', 'QA'].map((s, i) => (
            <span key={s} style={{
              fontSize: 8, fontFamily: 'var(--font-mono)', padding: '3px 8px', borderRadius: 4,
              background: i === 1 ? catAccent.subtle : C.cream,
              border: `1px solid ${i === 1 ? catAccent.border : T.lineDark}`,
              color: i === 1 ? catAccent.text : C.slate,
            }}>{s}</span>
          ))}
        </div>
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }} className="program-workflow-panels">
      <Panel label="INPUT" accent={accent}>
        <div style={{ fontSize: 11, color: C.slate, lineHeight: 1.5 }}>{steps[0]?.detail}</div>
      </Panel>
      <Panel label="OUTPUT" accent={accent} highlight>
        <div style={{ fontSize: 11, color: C.slate, lineHeight: 1.5 }}>{steps[steps.length - 1]?.detail}</div>
      </Panel>
    </div>
  )
}
  ...
  return (
    <GlassSurface level={3} padding={0} style={{ overflow: 'hidden' }} className="program-workflow-visual">
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 16px', borderBottom: `1px solid ${T.lineDark}` }}>
        ...
        <div style={{ flex: 1, textAlign: 'center', color: C.slate, fontSize: 11, fontFamily: 'var(--font-mono)', letterSpacing: '0.04em' }}>
            ...
            <div key={step.label} style={{ display: 'flex', alignItems: 'center', flex: '1 1 0', minWidth: 72 }}>
              <div style={{ flex: 1, textAlign: 'center', minWidth: 0 }}>
                <div style={{
                  ...
                  background: isActive ? accent.subtleStrong : isPast ? C.cream : C.cream,
                  border: `1px solid ${isActive ? accent.border : T.lineDark}`,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: 10, fontFamily: 'var(--font-mono)', color: isActive ? accent.text : C.slate,
                }}>
                  {String(i + 1).padStart(2, '0')}
                </div>
                <div style={{ color: isActive ? C.ink : C.slate, fontSize: 11, fontWeight: 600, lineHeight: 1.2, marginBottom: 2 }}>{step.label}</div>
                <div style={{ color: C.slate, fontSize: 9, lineHeight: 1.3 }}>{step.detail}</div>
              </div>
              {i < steps.length - 1 && (
