...
export function PageHero({
  ...
  return (
    <section style={{ background: bg, position: 'relative', overflow: 'hidden', padding: `clamp(120px, 16vw, 180px) ${T.gutter} clamp(72px, 9vw, 110px)` }}>
      <div style={{ maxWidth: T.maxW, margin: '0 auto', position: 'relative' }}>
        ...
        <FadeIn delay={80}>
          <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 'clamp(40px, 6.5vw, 88px)', lineHeight: 0.98, letterSpacing: '-0.035em', color: tone === 'dark' ? C.white : C.ink, margin: 0, maxWidth: 900 }}>
            {title}
          </h1>
        </FadeIn>
        {lead && (
          <FadeIn delay={160}>
            <p style={{ color: tone === 'dark' ? 'rgba(255,255,255,0.5)' : C.slate, fontSize: 'clamp(16px, 2vw, 20px)', lineHeight: 1.7, margin: '28px 0 0', maxWidth: 560 }}>{lead}</p>
          </FadeIn>
        )}
