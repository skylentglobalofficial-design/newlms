          ...
          <div>
            <div style={{ display: 'flex', gap: 6, marginBottom: 18 }}>
              {(['upi', 'card', 'netbanking', 'emi'] as const).map(m => (
                <button key={m} type="button" onClick={() => setPayMethod(m)} style={{ flex: 1, padding: '9px 4px', borderRadius: 7, border: `1px solid ${payMethod === m ? C.ink : 'rgba(11,13,15,0.15)'}`, background: payMethod === m ? C.ink : 'transparent', color: payMethod === m ? C.white : C.slate, fontSize: 11, cursor: 'pointer', fontFamily: 'var(--font-mono)', transition: 'all 0.2s', textTransform: 'uppercase' }}>{m}</button>
              ))}
            </div>
            {payMethod === 'upi' && (
              <div>
