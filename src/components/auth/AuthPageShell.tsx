...
export function AuthSubtitle({ children }: { children: React.ReactNode }) {
  return <p className="auth-subtitle">{children}</p>
}

export function AuthEnrollNotice({ kind }: { kind: "course" | "program" }) {
  return (
    <p className="auth-enroll-notice" role="status">
      Sign in to continue enrollment in this {kind}.
    </p>
  )
}
...
export function AuthError({ message, title = "Unable to sign in" }: { message: string; title?: string }) {
  return (
...
export function AuthGoogleIcon() {
  return (
    <svg className="auth-google-icon" width="18" height="18" viewBox="0 0 24 24" aria-hidden focusable="false">
